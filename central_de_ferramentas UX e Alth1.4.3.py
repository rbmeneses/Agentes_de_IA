...
if not authentication_status:
    ...
    def get_api_key(key_name):
        ...
        try:
            ...
            # Não exiba erro aqui, pois a função pode ser chamada para verificar a existência da chave
            # O erro será exibido na página específica que requer a chave.
            return None
    ...
    # A função foi mantida no escopo global para ser reutilizável.
    ...
    def page_gerador_exercicios():
        ...
        if not GEMINI_API_KEY:
            st.error("A chave da API Gemini é necessária para esta ferramenta. Configure-a em 'Perfil e Configurações'.")
            return
        if not GCP_PROJECT_ID:
            st.error("O Project ID do Google Cloud é necessário para gerar imagens. Configure-o em 'Perfil e Configurações'.")
            return
        if not GCP_LOCATION:
            st.error("A Location do Google Cloud é necessária para gerar imagens. Configure-a em 'Perfil e Configurações'.")
            return
        ...
        def ge_gerar_imagem_com_vertexai(prompt):
            try:
                # *** CORREÇÃO APLICADA AQUI ***
                # vertexai.init movido para dentro da função.
                ...
                if not resposta or not resposta[0]._image_bytes:
                    st.warning(f"A IA se recusou a gerar a imagem para o prompt: '{prompt}'. Isso pode ocorrer por restrições de segurança ou clareza do prompt.")
                    return None
        ...
        def generate_exercises(data, api_key):
            try:
                ...
                if data['include_images']:
                    ...
                    if image_prompts:
                        with st.spinner(f"Gerando {len(image_prompts)} imagem(ns)..."):
    ...
    def page_otimizador_prompt():
        ...
        def op_analisar_imagem_para_contexto(image_bytes):
            ...
            try:
                ...
                prompt_instrucao = "Descreva de forma concisa e objetiva o conteúdo principal desta imagem. Esta descrição será usada como contexto para criar um novo prompt de IA. Foque nos elementos, responda em portugues br, estilo e composição."
            ...
            if not content.strip():
                ...
                if uploaded_file:
                    with st.spinner("Analisando imagem de contexto..."):
                        ...
                        if not image_context_description:
                            st.warning("Falha ao analisar a imagem. O processo continuará sem o contexto visual.")

                # Etapa 1: Otimizar o prompt
                with st.spinner("Etapa 1: Otimizando seu prompt com IA..."):
                    prompt_for_gemini = (
                        f"Você é um engenheiro de prompts especialista. Sua tarefa é criar um prompt otimizado e detalhado em inglês porem as respostas devem ser em portugues br para uma IA de {'imagem' if media_type == 'Gerar Imagem' else 'texto'}. "
                ...
                with st.spinner(f"Etapa 2: Usando o prompt para gerar a {'imagem' if media_type == 'Gerar Imagem' else 'saída de texto'}..."):
                    if media_type == "Gerar Texto":
                        # Usar o prompt otimizado para gerar o texto final
                        ...
                        # Verificar se as chaves para geração de imagem estão disponíveis
                        if not GCP_PROJECT_ID or not GCP_LOCATION:
                            st.error("Para gerar imagens, o Project ID e a Location do Google Cloud devem ser configurados no seu perfil.")
                            return

                        # Usar o prompt otimizado para gerar a imagem final
    ...
    def page_analise_visual():
        ...
        if not GEMINI_API_KEY:
            ...
            return
        if not GCP_PROJECT_ID:
            st.error("O Project ID do Google Cloud é necessário para esta ferramenta. Configure-o em 'Perfil e Configurações'.")
            return
        if not GCP_LOCATION:
            st.error("A Location do Google Cloud é necessária para esta ferramenta. Configure-a em 'Perfil e Configurações'.")
            return
                ...
                with st.spinner("A IA está analisando a imagem..."):
                    try:
                        ...
                        if resultado_analise:
                            ...
                            # *** CORREÇÃO APLICADA AQUI ***
                            # Tratamento mais robusto para a divisão da saída da IA.
                            if "Geração de Código:" in resultado_analise and "Geração de Prompt:" in resultado_analise:
                                partes = resultado_analise.split("Geração de Prompt:")
                                codigo_gerado = partes[0].replace("Geração de Código:", "").strip()
                                prompt_otimizado = partes[1].strip()

                                st.subheader("Código Gerado a partir da Imagem:")
                                st.code(codigo_gerado, language='python') # Default to python, user can change if needed
                                st.subheader("Prompt Otimizado para Descrever a Imagem:")
                                st.code(prompt_otimizado, language='text')
                            else:
                                st.subheader("Saída da IA (formato inesperado):")
                                st.code(resultado_analise, language='text')
                                st.warning("A IA retornou um formato inesperado. Verifique a saída bruta.")
...
if __name__ == "__main__":
    ...
    def page_analise_logs():
        ...
        if not GEMINI_API_KEY:
            st.warning("A chave da API Gemini é necessária para esta ferramenta. Configure-a em 'Perfil e Configurações'.")
            return
        if not GCP_PROJECT_ID:
            st.warning("O Google Cloud Project ID é necessário para esta ferramenta. Configure-o em 'Perfil e Configurações'.")
            return
        if not GCP_LOCATION:
            st.warning("A Google Cloud Location é necessária para esta ferramenta. Configure-a em 'Perfil e Configurações'.")
            return
                ...
                with st.spinner("Analisando log e eventos..."):
                    try:
                        ...
                        if uploaded_image_log:
                            ...
                            if image_analysis_result and "Erro na análise visual da imagem" not in image_analysis_result: # Check for actual error message
                                st.success("Análise visual concluída.")
                            else:
                                st.warning("Não foi possível obter uma análise visual da imagem ou ocorreu um erro na análise.")
                                if "Erro na análise visual da imagem" in image_analysis_result:
                                    st.error(image_analysis_result) # Display the specific error from the function
                        ...
                        if image_analysis_result and "Erro na análise visual da imagem" not in image_analysis_result:
    ...
    def page_espelho_da_mente():
        ...
        def em_criar_prompt_avancado_com_ia(dados_usuario):
            ...
            if not GEMINI_API_KEY:
                st.error("A chave da API Gemini é necessária para criar o prompt. Configure-a em 'Perfil e Configurações'.")
                return None
            ...
            if not all([pensamento_usuario, cenario, objeto]):
                ...
                try:
                    ...
                    if prompt_final:
                        ...
                        st.error("Não foi possível gerar o prompt para a imagem. Verifique se a chave Gemini API está configurada.")
                except Exception as e:
    ...
    def page_buscador_vagas():
            ...
            if not GSEARCH_KEY:
                st.warning("A chave Google Custom Search API é necessária para esta ferramenta. Configure-a em 'Perfil e Configurações'.")
                return
            if not GSEARCH_CX:
                st.warning("O Google Custom Search CX ID é necessário para esta ferramenta. Configure-o em 'Perfil e Configurações'.")
                return
